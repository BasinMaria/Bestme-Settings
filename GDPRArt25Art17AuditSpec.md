...
| Профиль 13–17 открытый по умолчанию — **НЕЗАКОННО** | ❌ DSA Art.28(3)(g) — строго ON для несовершеннолетних |
...
| (f) | *"collected in relation to information society services for minors (Art.8)"* | Account → Delete account (работает для любого возраста) | ⚠️ Нужен специальный маршрут для родителей детей 13–17 |
...
> **Пробел по основанию (f)**: Для аккаунтов несовершеннолетних — механизм, позволяющий **родителю/опекуну** запросить удаление данных ребёнка.
...
| `account_private = ON` для 13–17 | ✅ Прописано | — |
...
| Art.17(1)(f): Несовершеннолетние | ⚠️ Частично | Родительский запрос |
...
- [ ] **Onboarding disclosure**: добавить явное уведомление о публичном профиле при регистрации взрослых
...
- [ ] **Age gate 13-17**: принудительно `account_private = true` при регистрации несовершеннолетних
