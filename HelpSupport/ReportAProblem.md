...
| `legal_hold` | Boolean | Да (по умолчанию `false`) | Замок для тяжелых нарушений (CSAE, терроризм). Если `true`, скрипт игнорирует `retention_expires_at` и ничего не удаляет. |

---

## 18. Сроки хранения данных (Data Retention)

> Все удаленные данные переходят в состояние **Soft Delete** (скрыты от пользователей). Физическое удаление (Hard Delete) происходит только по истечении указанных сроков.
...
* **Действие:** По закону DSA у нарушителя есть полгода на апелляцию. Пост скрывается, но хранится в базе вместе с фото/видео до конца срока, затем стирается полностью.
...
* **Действие:** Контент немедленно скрывается от пользователей. На саму жалобу, на удаленный контент (фото/видео) и на профиль нарушителя вешается флаг `legal_hold = true`. Скрипты автоудаления обходят эти данные стороной.
...
1. **Автоматическое принятие решений (GDPR Art. 22):** Мы используем автоматизированные системы (AI) для выявления спама и нелегального контента. Вы имеете право оспорить решение системы, запросив проверку человеком.
2. **Срок апелляций:** Если ваш контент удален за нарушение правил, мы сохраняем его скрытую копию (Soft Delete) в течение 6 месяцев для обеспечения вашего права на апелляцию.
3. **Legal Hold (Заморозка данных):** Платформа имеет право сохранять данные (IP, переписку, медиа) на срок до 7 лет при расследовании уголовных преступлений (например, CSAE) и передавать их правоохранительным органам (NCMEC, Europol).
...
5. **Модерация личных сообщений (DM):** Мы не читаем личные сообщения. Однако при жалобе на конкретное сообщение в чате модератор получает доступ к тексту только этого сообщения для принятия решения.

### В Terms of Service (Раздел Safety & Moderation / Правила сообщества):
1. **Права платформы (DSA Art. 14):** Мы оставляем за собой право скрывать, удалять контент или блокировать аккаунты, нарушающие Community Guidelines.
2. **Апелляции (DSA Art. 17, 20):** При удалении контента мы уведомляем о причине (Statement of Reasons). Пользователь может подать апелляцию в течение 6 месяцев через приложение.
3. **Система страйков:** Повторные нарушения (до 3-х предупреждений) ведут к блокировке аккаунта. При грубых нарушениях бан может быть выдан без предупреждений (Permanent Ban).
4. **Злоупотребление:** Использование системы жалоб для травли или массовой отправки ложных репортов ведет к блокировке аккаунта заявителя.
5. **Метки дезинформации:** Платформа вправе помечать спорный контент предупреждающими ярлыками (Disputed) или ограничивать его распространение в алгоритмической ленте.
...
*   **Reason for Ban:** Что вызвало блокировку (Лимит страйков / Категория репорта от юзеров / Либо просто `-`, если модератор забанил вручную).
...
Те, кто отбыл наказание или был разбанен после апелляции. Статус у всех `Active`. Здесь нам важно знать, **когда он был разбанен**.
...
*   `Report ID` и `Target User` в таблице — это **кликабельные ссылки**, которые перекидывают админа на уже готовую карточку (Раздел 1 и Раздел 4).
...
### Раздел 6: 🎫 Support Tickets (Обращения в поддержку)

> 📌 **Назначение:** Обработка запросов из формы `Contact Support`. Обращения могут быть как от авторизованных пользователей (передается ID), так и от неавторизованных гостей (связь только по Email).

Экран делится на 2 логические вкладки:
*   **Вкладка 1: 📥 Inbox (Открытые тикеты)** — Показывает новые обращения со статусом `New`, взятые в работу `In Progress` и переданные специалистам `Escalated`.
*   **Вкладка 2: 🗄️ Resolved (Архив)** — Показывает закрытые тикеты со статусом `Resolved` (решено) или `Dismissed`.

**Строка списка (List Item UI):**
```text
[ Ticket ID ]   [ Topic ]                  [ User / Email ]         [ Preview ]                  [ Date ]     [ Status ]
#SUP-1024       🐞 Bug or crash            alex@example.com         "Приложение вылетает..."     5 min ago    🟢 New
#SUP-1023       🔑 Account access          👤 Ivan (#90)            "Не могу сбросить пароль"    1 hour ago   🟡 In Progress
#SUP-1025       🗑️ Privacy/Data            guest@email.com          "Удалите мои данные"         1 day ago    🟠 Escalated
#SUP-1022       💡 Suggestion              anna@email.com           "Добавьте темную тему"       2 days ago   ⚪ Resolved
```

**Детальная карточка обращения (Support Ticket Detail):**

**Блок А: Базовая информация (Header)**
*   **Topic:** Тема обращения из формы (Bug or crash / Account access / Privacy & Data / Suggestion).
*   **Contact Info:** Email для обратной связи (обязательное поле). Для зарегистрированных пользователей поле автозаполняется.
*   **User:** Ссылка на профиль (если запрос отправлен залогиненным), иначе — `Guest`.
*   **Tech Data:** Версия приложения, OS (iOS/Android), Модель устройства (желательно передавать "под капотом").

**Блок Б: Содержание (Message Body)**
*   **Description:** Полный текст описания проблемы, который ввел пользователь.

**Панель действий (Action Bar):**

1. **✉️ Reply via Email (Ответить)** — Открывает форму (или модальное окно) для написания ответа пользователю на его Email.  
   **В окне отправки есть две логические кнопки:**
   ```text
   [ Send Reply & Keep Open ]   [ Send Reply & Mark Resolved ✅ ]
   ```
   * `[ Send Reply & Keep Open ]` — Письмо уходит, статус становится `In Progress` (ждем, что юзер ответит / проверяем).
   * `[ Send Reply & Mark Resolved ✅ ]` — Письмо уходит, проблема решена, статус меняется на `Resolved`, тикет сразу уходит во вкладку архива.
   
2. **✅ Mark as Resolved (Закрыть без ответа)** — Закрыть тикет "тихо". Применяется для спама, дубликатов или предложений (Suggestion), когда ответное письмо писать не нужно. Пользователь ничего не получает.

3. **🔄 Escalate / Forward (Передать)** — Передать тикет профильному специалисту (например, разработчикам для фикса багов, или DPO для удаления данных по GDPR). Тикет меняет статус на `🟠 Escalated` (Передано) и уходит в очередь соответствующего отдела. После решения специалист отвечает пользователю и закрывает тикет.

---

#### Как пользователи получают ответы (Communication Flow)

*   Ответы саппорта — это **транзакционные (сервисные) сообщения**. Отдельное согласие (галочка) на их получение не требуется, так как пользователь сам инициировал запрос и ждет ответа.
*   Email, указанный в форме, используется только для решения проблемы и не добавляется в маркетинговые рассылки.

**Шаблон системного Email-ответа (Support Reply):**
> **От кого:** support@bestme.app
> **Кому:** [Email из формы]
> **Тема:** Re: [Topic] — Support Ticket #[ID]
>
> Здравствуйте! / Hello,
>
> [ Текст ответа модератора / саппорта ]
>
> ---
> Если у вас остались вопросы, просто ответьте на это письмо.
> С уважением, Команда поддержки Bestme
>
> ---
> *Ваш исходный запрос (Your original request):*
> **Тема:** [Topic]
> **Описание:** [Description из формы]
