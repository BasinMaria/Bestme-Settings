...
**Обязательно:**
⚖️ DSA Art.14 (механизм жалоб на незаконный контент) · DSA Art.16 (уведомление репортёра о результате) · DSA Art.17 (Statement of Reasons) · DSA Art.20 (Апелляции)
...
> ⚖️ **GDPR Art.22:** запрещено принимать юридически значимые решения исключительно автоматически без возможности вмешательства человека.  
...
| `retention_expires_at` | Timestamp | Нет | Точная дата, когда скрипт должен навсегда удалить этот репорт (например, `resolved_at` + 180 дней). |
| `legal_hold` | Boolean | Да (по умолчанию `false`) | Замок для тяжелых нарушений (CSAE, терроризм). Если `true`, скрипт игнорирует `retention_expires_at` и не удаляет запись. |

---

## 18. Сроки хранения данных (Data Retention)

> Все удаленные данные переходят в состояние **Soft Delete** (скрыты от пользователей). Физическое удаление (Hard Delete) происходит автоматически по расписанию (CRON).
...
* **Действие:** По закону DSA у нарушителя есть полгода на апелляцию. Пост скрывается, но хранится в базе вместе с жалобой. Через 6 месяцев скрипт удаляет контент физически, а логи анонимизируются (удаляются user_id).
...
* **Действие:** Контент немедленно скрывается от пользователей. На саму жалобу, на удаленный контент (фото/видео) и на профиль нарушителя вешается флаг `legal_hold = true`. Автоматическое удаление блокируется. Данные хранятся для передачи правоохранительным органам.
...
1. **Автоматическое принятие решений (GDPR Art. 22):** Мы используем автоматизированные системы (AI) для выявления спама и незаконного контента. Если ваш контент был удален автоматически, вы имеете право запросить пересмотр решения модератором.
...
3. **Legal Hold (Заморозка данных):** Платформа имеет право сохранять данные (IP, переписку, медиа) на срок до 7 лет при подозрении на уголовные преступления для передачи полиции, даже при удалении аккаунта пользователем.
4. **Конфиденциальность жалоб:** Жалобы строго анонимны. Личность заявителя (Reporter) не раскрывается нарушителю.
5. **Модерация личных сообщений (DM):** Мы не читаем личные сообщения. Однако при жалобе на конкретное сообщение модераторы получают доступ к этому сообщению и контексту чата.

### В Terms of Service (Раздел Safety & Moderation / Правила сообщества):
1. **Права платформы (DSA Art. 14):** Мы оставляем за собой право скрывать, удалять контент или блокировать аккаунты, нарушающие Правила сообщества (Community Guidelines).
2. **Апелляции (DSA Art. 17, 20):** При удалении контента мы уведомляем о причине (Statement of Reasons). Пользователь может подать апелляцию в течение 6 месяцев.
3. **Система страйков:** Повторные нарушения (до 3-х предупреждений) ведут к блокировке аккаунта. При грубых нарушениях (CSAE, терроризм, мошенничество) применяется немедленная пожизненная блокировка (Zero Tolerance).
...
5. **Метки дезинформации:** Платформа вправе помечать спорный контент предупреждающими ярлыками (Disputed) или ограничивать его видимость.
...
*   **🗑️ Remove (Удалить)** — Контент скрывается (soft delete). Статус меняется на `Removed`. Уведомления нарушителю и репортеру.
*   **🚫 Ban User (Забанить)** — Контент скрывается. Юзер получает бан. Статус меняется на `Banned`. Уведомления нарушителю и репортеру.
*   **👁️‍🗨️ Dismiss (Отклонить)** — Контент остается. Нарушений нет. Статус меняется на `Dismissed`. Уведомление только репортеру.
...
### Раздел 4: 🚫 Suspended Users (Заблокированные аккаунты)
Один простой список с фильтром: `Active Bans` / `Permanent Bans`.

**Детальная карточка нарушителя (User Profile CRM):**
Здесь модератор видит всю историю юзера (страйки, сколько репортов, сколько раз его заблокировали другие юзеры — метрика `Blocked By` из таблицы `user_blocks`).
*   **🔓 Unban (Разбанить)** — Снимает ограничения, обнуляет страйки. Статус `Active`.
*   **⏳ Suspend (Временный бан)** — Временный бан от 7 до 30 дней. Статус `Suspended`.
*   **⬛ Permanent Ban (Пермабан)** — Полное удаление аккаунта навсегда (бан по IP/Device). Статус `Banned`.
*   **🔒 Legal Hold (Заморозка)** — Замораживает аккаунт для полиции до 7 лет. Статус `Legal_Hold` (без уведомления юзера).
...
*   `Report ID` и `Target User` в таблице — это **кликабельные ссылки**, которые перекидывают админа на уже готовую карточку тикета во вкладку *History* или в профиль юзера.
