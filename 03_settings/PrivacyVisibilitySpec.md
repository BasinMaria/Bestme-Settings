...
10. [Что добавлено в v2.5](#changelog-v25)
...
> 📎 Для настроек **алгоритмических рекомендаций** (отказ от алгоритма, «Почему мне это рекомендуют?») — см. [§2.6 Discoverability](#26-discoverability).
...
> Управляет тем, **как алгоритм рекомендует профиль** пользователя другим.  
> Настройки поиска и SEO-индексации (`profile_searchable`, `seo_indexable`) находятся в **[§2.1 Account Privacy](#21-account-privacy)** — там задаётся единое значение, которое используется по всему приложению.  
> Дублировать их здесь не нужно: одна переменная — одно место хранения и редактирования.
...
|---|---|---|---|
| 1 | **`seo_indexable = false` по умолчанию** — нельзя менять | §2.1 | GDPR Art.25 — штраф до 10 млн € |
...
<a name="changelog-v25"></a>

> 📋 **Для разработчиков:** изменения относительно v2.4.

### ✅ ЧТО ДОБАВЛЕНО / ИЗМЕНЕНО в v2.5

| # | Что | Куда | Причина |
|---|---|---|---|
| 1 | **`profile_searchable` и `seo_indexable` удалены из §2.6** | §2.6 Discoverability | Эти поля уже определены в §2.1 Account Privacy. Одна переменная — одно место. Дублирование вводило в заблуждение: разработчики могли создать два отдельных поля в БД вместо одного |
| 2 | **Описание §2.6 обновлено** | §2.6 Discoverability | Раздел теперь чётко сфокусирован на алгоритмических рекомендациях; добавлена перекрёстная ссылка на §2.1 |
| 3 | **Примечание в §2.1 добавлено** | §2.1 Account Privacy | Обратная ссылка: §2.1 = поиск/SEO; §2.6 = рекомендации алгоритма |
| 4 | **ASCII-дерево обновлено** | Блок Discoverability | Удалены строки `Profile in search` и `SEO indexing` — остались только `Opt-out recommendations` и `Why recommended?` |
| 5 | **"Обязательно для публикации"** обновлено | Таблица блокеров | Ссылка на `seo_indexable` исправлена с "§2.1 + §2.6" → "§2.1" |

> 💡 **Почему это правильно?** Принцип «одна переменная — одно место» (Single Source of Truth): если поле `seo_indexable` отображалось в двух разделах, UI мог отображать разные значения (после изменения в одном месте). Теперь §2.1 = единственное место редактирования.
