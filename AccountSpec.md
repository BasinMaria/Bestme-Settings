...
5. [Логика размещения полей](#логика-размещения-полей)
6. [ASCII-структура раздела](#ascii-структура)
7. [Обязательно для публикации](#обязательно-для-публикации)
8. [Что изменено в v1.0 — сводка для разработчиков](#changelog-v10)

---

## Логика размещения полей

> **Вопрос, который задают чаще всего:** «Почему Interests в User Info, а не в Profile Info? Почему Goals в Profile Info? Почему Language здесь?»

### Принцип разделения

Всего в разделе Account четыре подраздела. Каждый отвечает за свой тип данных:

| Подраздел | Что хранит | Ключевой признак |
|---|---|---|
| **§1.1 User Info** | Структурные / системные данные | Используются **алгоритмом**, для **идентификации**, отображаются на **карточке** пользователя |
| **§1.2 Profile Info** | Описательные / выразительные данные | Пользователь описывает **себя словами** или выбирает **личные намерения** |
| **§1.3 Contact Info** | Контактные данные | Где с тобой связаться |
| **§1.4 Account Management** | Жизненный цикл аккаунта | Создать / переключить / удалить |

---

### §1.1 User Info — почему именно эти поля?

User Info = данные, которые **система использует структурно**:

| Поле | Почему здесь |
|---|---|
| **Avatar / Cover** | Отображается на карточке, в ленте, в поиске — часть визуальной идентичности |
| **Status** | Показывается **на аватарке** — неотделим от аватара UX-но |
| **First name / Last name** | Идентификация пользователя в системе, защита от спуфинга |
| **Date of birth** | Возрастная верификация (18+), COPPA/GDPR Art.8 — система должна знать, допущен ли пользователь |
| **Gender** | Структурный атрибут, влияет на рекомендации и персонализацию |
| **Interests** | **Категории wellness (6 штук)** — система использует их для: алгоритма фида, тематических страниц, поиска, рекомендаций. Это **структурный тег**, не описательный текст. Как «жанр» для книги — он определяет, куда тебя помещает система. |

> ⚠️ **Почему Interests — не в Profile Info?**  
> Потому что Interests — это **не то, что ты пишешь о себе**, а **то, как система тебя классифицирует**.  
> Interests = алгоритмический инпут, как Gender или Date of birth.  
> Profile Info = то, что ты **сам выражаешь** о себе в свободной форме.

---

### §1.2 Profile Info — почему именно эти поля?

Profile Info = данные, которые пользователь **выражает сам** — описывает себя или выбирает личные намерения:

| Поле | Почему здесь |
|---|---|
| **Bio / About** | Свободный текст «о себе» — пользователь пишет что хочет (≤ 300 символов). Чисто выразительное поле. |
| **Goals** | Личные цели пользователя — что он **хочет достичь** в приложении. ⚠️ **Goals НЕ связаны с Interests-категориями** — пользователь выбирает любую цель из общего списка (или создаёт свою), независимо от выбранных категорий. Goals = инструмент вовлечённости (engagement): они мотивируют пользователя к действию внутри приложения («пить больше воды», «больше ходить», «медитировать»). |
| **Language** | Язык интерфейса приложения для этого пользователя. Это UX-предпочтение «как я хочу видеть приложение», а не системный атрибут. |

---

### Краткое правило для дизайнеров и разработчиков

> **User Info** → «Система должна это знать, чтобы работать правильно»  
> **Profile Info** → «Пользователь выражает себя / своё намерение»
...
> **Критерий:** система использует эти данные структурно (алгоритм, идентификация, возрастная верификация).  
...
> Дополнительная информация, которую пользователь **выражает сам** — описывает себя или выбирает личные намерения.  
> **Критерий:** пользователь свободно описывает себя или выбирает то, что он *хочет*, а не то, что нужно системе.  
...
> 💡 **Goals (цели)** — личные цели пользователя: что он хочет достичь с помощью приложения.  
> ⚠️ **Goals НЕ связаны с Interests-категориями** — пользователь выбирает любую цель независимо от своих категорий.  
> Goals — это инструмент вовлечённости: они мотивируют пользователя к действию («пить больше воды», «больше ходить», «медитировать»).  
> Подробнее о различии — см. [§ Логика размещения полей](#логика-размещения-полей).
...
└── 1️⃣ 👤 Account  ⚖️ App Store §5.1.1 · Google Play · GDPR Art.17
    ...
    ├── Profile Info
    │   ├── Bio / About
    │   ├── Goals  (engagement: любые личные цели — НЕ привязаны к категориям Interests)
    │   └── Language
    │
...
| 8 | **Goals — исправлено описание** (v1.2) | §1.2 Profile Info | "Goals" | Убрана ошибочная привязка Goals к категориям Interests. **Goals НЕ зависят от Interests** — пользователь выбирает любую цель из общего списка, независимо от выбранных категорий. Goals = инструмент вовлечённости (engagement), мотивируют к действию. |
| 9 | **Логика размещения полей добавлена** (v1.2) | §0 (новый) | — | Добавлен раздел «Логика размещения полей» с объяснением принципа: User Info = структурные данные для системы (алгоритм/идентификация), Profile Info = выразительные данные (пользователь описывает себя/намерения). Объяснено, почему Interests — в User Info, а не в Profile Info. |

---

*AccountSpec.md v1.2 · Bestme · март 2026*  
